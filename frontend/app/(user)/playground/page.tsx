...
export default function PlaygroundPage() {
  ...
  const handleAnalyseAll = useCallback(() => {
    ...
    const source = decodeModelKey(sourceKey);
    ...
    if (mode === "historical") {
      ...
      if (trafficMultiplier)
        params.set("traffic_multiplier", trafficMultiplier);
    } else {
      if (requestCount) params.set("request_count", requestCount);
      if (avgPromptTokens) params.set("avg_prompt_tokens", avgPromptTokens);
      if (avgCompletionTokens)
        params.set("avg_completion_tokens", avgCompletionTokens);
    }

    router.push(`/playground/analyse-all?${params.toString()}`);
  }, [
    avgCompletionTokens,
    avgPromptTokens,
    endDate,
      ...
      try {
        ...
        const initialSource = payload.recent_models[0]
          ...
          : payload.catalog[0]
            ...
            : "";
        const initialTarget = initialSource
          ? pickDefaultTarget(payload.catalog, initialSource)
          : "";
    ...
    return (
      <div className="mx-auto mt-12 flex max-w-[500px] flex-col items-center justify-center space-y-4 rounded-xl border border-white/10 bg-[#0a0a0a] p-10 text-center shadow-sm">
        ...
        <p className="font-sans text-sm text-zinc-400">
          Select a project before opening the model playground to track
          potential what-if scenarios based on recorded real-world constraints.
        </p>
      </div>
    );
  }
    ...
    <div className="mx-auto flex max-w-7xl flex-col space-y-8 pb-10">
      <section className="flex flex-col gap-2">
        ...
        <p className="max-w-2xl text-sm font-medium text-zinc-400">
          Compare model costs against real traffic before shipping. Use your
          project's observed usage or build a manual scenario.
        </p>
      </section>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          value={
            result
              ? result.delta.savings_usd >= 0
                ? `$${result.delta.savings_usd.toFixed(4)}`
                : `-$${Math.abs(result.delta.savings_usd).toFixed(4)}`
              : "--"
          }
          tone={
            result && result.delta.savings_usd < 0 ? "destructive" : "neutral"
          }
          icon={
        ...
        <div className="flex flex-col space-y-4">
          <div className="flex items-center justify-between pb-1 border-b border-white/5">
            ...
            <button
              ...
              className="cursor-pointer text-zinc-500 hover:text-white transition-colors disabled:opacity-50 pb-1"
              title="Refresh comparison"
            >
              <ArrowsClockwise
                size={16}
                weight="regular"
                className={runningCompare ? "animate-spin" : ""}
              />
            </button>
          </div>
          ...
          <div className="flex flex-col gap-2 relative">
            {loadingOptions ? (
              Array.from({ length: 4 }).map((_, i) => (
                <div
                  key={i}
                  className="h-[84px] animate-pulse rounded-lg bg-white/5 border border-white/5"
                />
              ))
              ...
              options.recent_models.map((entry) => {
                ...
                return (
                  <button
                    ...
                    className={`cursor-pointer flex flex-col justify-between rounded-lg border p-3 text-left transition-all relative ${isActive ? "border-white/20 bg-white/5 ring-1 ring-white/10" : "border-white/5 bg-transparent hover:border-white/10 hover:bg-white/[0.02]"}`}
                  >
                    <div className="flex w-full items-center justify-between">
                      ...
                      <span className="text-[10px] font-medium text-zinc-500">
                        {entry.event_count.toLocaleString()} req
                      </span>
                    </div>
                    <span className="mt-1.5 text-xs font-medium text-zinc-100 truncate w-full">
                      {entry.model}
                    </span>
              ...
              <div className="rounded-lg border border-dashed border-white/10 p-6 text-center">
                <p className="text-xs text-zinc-400">
                  No tracked models yet. Connect your app to see historical
                  models here.
                </p>
              </div>
            )}
              ...
              <div className="grid gap-5 md:grid-cols-3">
                ...
                <Field label="Avg prompt tokens">
                  <Input
                    ...
                    onChange={(event) => setAvgPromptTokens(event.target.value)}
                    className={inputClass}
                  />
                </Field>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-white/5">
              <div className="flex items-start gap-2.5 sm:mt-0">
                ...
                <p className="text-xs text-zinc-500 max-w-sm">
                  Historical mode scales actual tracked traffic data across time
                  periods. Manual mode provisions standard scenario blocks.
                </p>
              </div>
              <Button
      ...
      <Button
        ...
        disabled={!sourceKey}
        className="fixed right-6 bottom-6 z-40 h-11 rounded-xl border border-white/10 bg-white px-5 text-sm font-semibold text-black shadow-[0_12px_40px_rgba(0,0,0,0.4)] transition hover:bg-zinc-200"
      >
        Analyse All
      </Button>
    </div>
...
function MetricCard({
  ...
  return (
    <div className="flex flex-col rounded-xl border border-white/10 bg-white/2 inset-shadow-2xs inset-shadow-white/10 p-5 shadow-sm">
      ...
      <div
        className={`text-[2rem] leading-none font-semibold tracking-tight ${valueColor}`}
      >
        {value}
      </div>
    </div>
    ...
    <div className="flex flex-col rounded-xl border border-white/10 bg-white/2 inset-shadow-2xs inset-shadow-white/10 p-5 shadow-sm space-y-4">
      <div className="flex items-start gap-4 justify-between">
        <div className="flex flex-col overflow-hidden">
          <span className="text-[10px] uppercase font-semibold text-zinc-500 tracking-widest mb-1.5">
            {title}
          </span>
  ...
  return (
    <div className="flex flex-col rounded-xl border border-white/10 bg-white/2 p-5 inset-shadow-2xs inset-shadow-white/10 shadow-sm space-y-4">
      <div className="flex items-center justify-between">
        <span className="text-[10px] uppercase font-semibold text-zinc-500 tracking-widest">
          Delta
        </span>
        {cheaper ? (
      ...
      <div className="flex flex-col">
        <span
          className={`text-[2rem] leading-none font-semibold tracking-tight ${cheaper ? "text-emerald-400" : "text-rose-400"}`}
        >
          {cheaper ? "-" : "+"}$
          {Math.abs(result.delta.absolute_cost_change_usd).toFixed(4)}
        </span>
        <span className="text-xs font-medium text-zinc-500 mt-2">
          {cheaper
            ? "Projected footprint reduction."
            : "Projected footprint increase."}
        </span>
      </div>
      <div className="h-px w-full bg-white/5" />
      <div className="grid grid-cols-2 gap-2">
        <MiniStat
          label="Savings"
          value={`$${result.delta.savings_usd.toFixed(4)}`}
        />
        <MiniStat
          label="Percent"
          value={
            result.delta.percentage_cost_change != null
              ? `${result.delta.percentage_cost_change > 0 ? "+" : ""}${result.delta.percentage_cost_change.toFixed(2)}%`
              : "n/a"
          }
        />
