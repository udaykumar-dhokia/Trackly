...
import { fetchProjects, createProject, setActiveProject } from "@/lib/store/features/projectsSlice"
...
export default function OrganizationsPage() {
  ...
  const { items: projects, status, error, activeProjectId } = useAppSelector((state) => state.projects)
  ...
  const handleCreateProject = async (e: React.FormEvent) => {
    ...
    setIsCreating(true)
    try {
      await dispatch(createProject({ orgId: user.org_id as string, name: newProjectName })).unwrap()
      setNewProjectName("")
    ...
    <div className="max-w-6xl mx-auto space-y-12">
      ...
      <section className="flex flex-col gap-2">
        ...
        <p className="text-zinc-400 font-mono text-sm max-w-2xl leading-relaxed">
          Manage your top-level organization and underlying projects. Tracking data and API keys are isolated within a Project to keep environments separate.
        </p>
      </section>

      {/* ORGANIZATION DETAILS BRUTALIST CARD */}
      <section>
        <div className="border border-white/10 bg-[#141418] p-8 relative overflow-hidden shadow-[6px_6px_0_0_#4f46e5]">
          <div className="flex flex-col gap-4 relative z-10">
            <div>
              <p className="text-xs uppercase tracking-widest text-zinc-500 font-semibold mb-1">Current Organization ID</p>
            ...
            <div>
              <p className="text-xs uppercase tracking-widest text-zinc-500 font-semibold mb-1">Owner</p>
      ...
      <section className="space-y-6">
        <div className="flex items-center justify-between border-b border-white/10 pb-4">
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent className="border-2 border-white/10 bg-[#141418] text-white">
                ...
                <form onSubmit={handleCreateProject} className="flex flex-col gap-4 mt-2">
                    <div>
        ...
        {status === 'loading' && <p className="text-zinc-500 font-mono animate-pulse">Loading active projects...</p>}
        {status === 'failed' && <p className="text-red-400 font-mono italic">Failed to load projects: {error}</p>}
        ...
        {status === 'succeeded' && projects.length === 0 && (
          <div className="border border-dashed border-white/20 p-12 flex flex-col items-center justify-center text-center bg-[#141418]/50">
            ...
            <p className="text-zinc-500 text-sm max-w-sm mb-6">Create your primary project below to start tracking API events and monitoring LLM costs.</p>
          </div>
        )}
              ...
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="font-bold text-lg text-white truncate pr-4">{proj.name}</h3>
                    {activeProjectId === proj.id && (
                  ...
                  <p className="font-mono text-[10px] text-zinc-500 break-all mb-4">ID: {proj.id}</p>
                </div>
                ...
                <button
                  ...
                  `}
                >
                  {activeProjectId === proj.id ? 'Currently Selected' : 'Select Project'}
