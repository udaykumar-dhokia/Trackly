...
import { fetchApiKeys, createApiKey, revokeApiKey, clearNewKeyDisplay } from "@/lib/store/features/apiKeysSlice"
import { Key, Plus, Trash, WarningCircle, CheckCircle, Copy } from "@phosphor-icons/react"

export default function ApiKeysPage() {
    const { user } = useUser()
    const dispatch = useAppDispatch()
    const { items: keys, status, newKeyDisplay } = useAppSelector((state) => state.apiKeys)
    ...
    const handleCreateKey = async (e: React.FormEvent) => {
        ...
        setIsCreating(true)
        try {
            const payloadProjectId = selectedProjectId === "none" ? null : selectedProjectId;
        ...
        <div className="max-w-6xl mx-auto space-y-10">
            ...
            <section className="flex flex-col gap-2">
                ...
                <p className="text-zinc-400 font-mono text-sm max-w-3xl leading-relaxed">
                    Manage organizational API keys used to authenticate requests to the Trackly ingest API.
                    Keys can be scoped to specific projects to cleanly separate traffic.
                </p>
            </section>

            {/* NEW KEY DISCLOSURE (SHOWN ONLY ONCE) */}
            {newKeyDisplay && (
                <section className="border-4 border-emerald-500 bg-emerald-500/10 p-8 shadow-[8px_8px_0_0_#10b981] space-y-4">
                    ...
                    <p className="text-zinc-300 font-mono text-sm">
                        Please copy this key immediately. You will <strong>never</strong> be able to see it again.
                    </p>
                    ...
                    <button onClick={() => dispatch(clearNewKeyDisplay())} className="text-sm border-b border-emerald-500/50 text-emerald-400 hover:text-emerald-300 transition-colors mt-4">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    {status === 'loading' && <div className="text-zinc-500 font-mono animate-pulse">Loading secure keys...</div>}
                    ...
                    {status === 'succeeded' && keys.length === 0 && (
                        <div className="border border-dashed border-white/20 p-12 flex flex-col items-center justify-center text-center bg-[#141418]/50">
                            ...
                            <p className="text-zinc-500 text-sm max-w-sm mb-6">Create an API key to start authenticating your backend traffic.</p>
                        </div>
                    )}
                    ...
                    {status === 'succeeded' && keys.length > 0 && (
                        <div className="space-y-4">
                            {keys.map(key => {
                                const linkedProject = projects.find(p => p.id === key.project_id)
                                return (
                                    <div key={key.id} className={`p-6 border-2 transition-all flex items-center justify-between
                          ...
                          `}>
                                        <div className="space-y-2">
                                            <div className="flex items-center gap-3">
                                                ...
                                                {!key.is_active && <span className="text-[10px] uppercase font-bold text-red-400 border border-red-500/30 px-2 py-0.5 bg-red-500/10">Revoked</span>}
                                            </div>
                                            <div className="flex items-center gap-4 text-xs font-mono">
                                                <span className="text-amber-300/80 tracking-wider">Prefix: {key.key_prefix}...</span>

                                                {key.project_id ? (
                                                    <span className="text-indigo-400 px-2 py-0.5 bg-indigo-500/10 border border-indigo-500/20">
                                                        Project: {linkedProject ? linkedProject.name : key.project_id.split('-')[0]}
                                                    </span>
                                                ) : (
                                            ...
                                            <p className="text-xs text-zinc-500">Created: {new Date(key.created_at).toLocaleDateString()}</p>
                                        </div>

                                        {key.is_active && (
                                            <button onClick={() => handleRevoke(key.id)} className="text-zinc-500 hover:text-red-400 transition-colors p-2 border border-transparent hover:border-red-500/30">
                                                <Trash size={20} />
                                            </button>
                                        )}
                                    </div>
                ...
                <div className="lg:col-span-1">
                    <div className="border-2 border-white/10 bg-[#141418] p-6 sticky top-8 shadow-[6px_6px_0_0_rgba(255,255,255,0.05)]">
                        <h3 className="text-xl font-bold text-white mb-6 border-b border-white/10 pb-4">Generate API Key</h3>
                        <form onSubmit={handleCreateKey} className="flex flex-col gap-6">

                            <div className="space-y-2">
                                <label htmlFor="keyName" className="text-sm font-bold text-zinc-300 uppercase tracking-widest">Key Name</label>
                                <input
                                    id="keyName"
                                    type="text"
                            ...
                            <div className="space-y-2">
                                <label htmlFor="projectScope" className="text-sm font-bold text-zinc-300 uppercase tracking-widest flex items-center justify-between">
                                    Scope
                                ...
                                <div className="text-xs text-zinc-500 pt-1 flex gap-2">
                                    ...
                                    <p>Keys without a project scope cannot log events, they can only manage Top-Level models.</p>
                                </div>
                            </div>
                            ...
                            >
                                ...
                                {!isCreating && <Plus weight="bold" className="group-hover:rotate-90 transition-transform" />}
                            </button>
                        </form>
                    </div>
                </div>
