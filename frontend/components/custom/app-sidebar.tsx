...
export function AppSidebar({ user, ...props }: React.ComponentProps<typeof Sidebar> & UserProps) {
    const { isMobile } = useSidebar()
    const pathname = usePathname()

    // Global initializers for the specific Organization and Projects context
    const dispatch = useAppDispatch()
    const { status } = useAppSelector(state => state.projects)

    useEffect(() => {
        if (user?.org_id && status === 'idle') {
    ...
    return (
        <Sidebar collapsible="icon" className="border-r border-white/10" {...props}>
            <SidebarHeader className="border-b border-white/5 pb-4 pt-4">
                <SidebarMenu>
                    <SidebarMenuItem>
                        <SidebarMenuButton size="lg" className="hover:bg-white/5 cursor-default">
                            <div className="grid flex-1 text-left text-sm leading-tight">
                                <span className="truncate font-semibold text-zinc-100">Trackly</span>
                            </div>
                        </SidebarMenuButton>
                    </SidebarMenuItem>
                </SidebarMenu>
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                <SidebarMenuItem key={item.title}>
                                    ...
                                    >
                                        <a href={item.url}>
                                            <item.icon weight={isActive ? "fill" : "duotone"} className={isActive ? "text-black size-5" : "text-zinc-400 size-5"} />
                    ...
                    <SidebarMenuItem>
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                ...
                                >
                                    <Avatar className="h-8 w-8 rounded-lg bg-[#141418] border border-white/10">
                            ...
                            >
                                ...
                                <DropdownMenuSeparator className="bg-white/10" />
                                <DropdownMenuGroup>
                                    <DropdownMenuItem asChild className="hover:bg-red-500/20 focus:bg-red-500/20 text-red-400 focus:text-red-300 cursor-pointer">
